    ...
    public class LineObjectPlacement : MonoBehaviour
    {
        ...
        private void PlacePrefabs()
        {
            ...
            for (int i = 0; i < LinePoints.Length; i++)
            {
                ...
                LineSegment[] inBetweenSegments = Array.FindAll(segments, x => x is { IsCorner: false, IsEndpoint: false });
                ...
                if (i < LinePoints.Length - 1)
                {
                    ...
                    while (placedLength <= targetDistance)
                    {
                        ...
                        if (segment.Length == 0)
                        {
                            Debug.LogError("Segment " + segment + " lengt was 0! Length must be set to a larger value!");
                            return;
                        }
                        ...
                        Vector3 segmentPosition = new Vector3(segmentStart.x, CorrectedSegmentHeight(segmentStart, segmentEnd).y, segmentStart.z);
                        Instantiate(segment.Prefab, segmentPosition + (Quaternion.LookRotation(direction) * segment.positionCorrection), rotation * Quaternion.Euler(segment.rotationCorrection), LinePoints[i]);
